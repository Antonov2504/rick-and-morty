...
export const useInfinityScrollWithSort = <T extends { created: string }>({ url }: UseInfinityScrollProps) => {
  ...
  const refLastNode = useCallback((node: Element) => {
    if (isLoading) return;
  ...
  }, [isLoading, sortedData.length, data?.info?.next, currentSort]);
